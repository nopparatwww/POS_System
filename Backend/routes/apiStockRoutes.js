    ...
    try {
      ...
      if (!productId || !quantity || isNaN(numQty) || numQty <= 0) {
        return res
          .status(400)
          .json({
            message: "productId และ quantity (ตัวเลข > 0) เป็นฟิลด์บังคับ",
          });
      }

      // Validate productId format to avoid Mongoose CastError
      ...
      // สร้าง ActivityLog กลาง ด้วย helper เพื่อให้พฤติกรรมเหมือนกันกับ routes อื่น ๆ
      try {
        const { logActivity } = require("../utils/activityLogger");
        await logActivity(req, "stock.in", 201, {
  ...
  async (req, res) => {
    try {
      ...
      if (!productId || !quantity || !reason) {
        return res
          .status(400)
          .json({ message: "productId, quantity, และ reason เป็นฟิลด์บังคับ" });
      }
      const numQty = parseFloat(quantity);
      if (isNaN(numQty) || numQty <= 0) {
        return res
          .status(400)
          .json({ message: "Quantity (จำนวน) ต้องเป็นบวก" });
      }
      ...
      // 2. ค้นหาสินค้า
      // Validate productId format to avoid CastError
      if (!mongoose.Types.ObjectId.isValid(productId)) {
        return res.status(400).json({ message: 'Invalid productId format' });
      }
      ...
      if (!product) {
        return res
          .status(404)
          .json({ message: "ไม่พบสินค้า (Product) ที่ระบุ" });
      }
      ...
      try {
        const { logActivity } = require("../utils/activityLogger");
        await logActivity(req, "stock.out", 201, {
          productId: updatedProduct._id,
          sku: updatedProduct.sku,
          quantity: numQty,
          reason: reason.trim(),
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Server error fetching stock out logs." });
    }
  }
);
    ...
    try {
      ...
      if (!productId || isNaN(numActual) || numActual < 0) {
        return res
          .status(400)
          .json({
            message: "productId และ actualStock (ตัวเลข >= 0) เป็นฟิลด์บังคับ",
          });
      }
      ...
      // Create central ActivityLog via helper for consistency
      try {
        const { logActivity } = require("../utils/activityLogger");
        await logActivity(req, "stock.audit", 201, {
        ...
        console.error("Failed to create activity log for stock.audit:", logErr);
      }
