...
// Role baseline mapping (keep in sync with middleware/ensurePermission and apiPermissionRoutes)
const roleBaseline = {
  admin: [
    "admin.dashboard",
    "admin.permissions",
...
function pickBody(b = {}) {
  ...
  if (b.reorderLevel != null)
    body.reorderLevel = Math.max(0, Number(b.reorderLevel));
  return body;
}
...
router.get(
  ...
  authenticateToken,
  ensureWithinShift,
  ensurePermission(["admin.products", "warehouse.products", "sales.products"]),
  async (req, res) => {
    try {
      ...
      if (qRaw) {
        const qr = new RegExp(`^${escapeRegExp(qRaw)}`, "i");
        criteria.$or = [
          { sku: qr },
          { name: qr },
          { category: qr },
...
router.get(
  ...
  authenticateToken,
  ensureWithinShift,
  ensurePermission(["admin.lowstock", "warehouse.lowstock"]),
  async (req, res) => {
    try {
      let items;
      try {
        ...
        console.warn(
          "Product lowstock: $expr query failed, falling back to in-memory filter:",
          dbExprErr && dbExprErr.message
        );
      ...
      res
        .status(500)
        .json({ message: "Server error fetching low stock items." });
    }
  }
);
...
router.get("/lowstock-robust", authenticateToken, async (req, res) => {
  try {
    console.log("[lowstock-robust] Incoming request:", {
    ...
    let permDoc = null;
    try {
      permDoc = await Permission.findOne({ user: me._id }).lean();
    } catch (e) {
    ...
    try {
      ...
      console.warn(
        "lowstock-robust: $expr failed, falling back to in-memory filter:",
        dbExprErr && dbExprErr.message
      );
      const all = await Product.find().sort({ name: 1 }).lean();
      items = all.filter((p) => {
    ...
    return res
      .status(500)
      .json({ message: "Server error fetching low stock items (robust)." });
  }
});
...
router.get(
  ...
  authenticateToken,
  ensureWithinShift,
  ensurePermission(["admin.products", "warehouse.products"]),
  async (req, res) => {
    try {
