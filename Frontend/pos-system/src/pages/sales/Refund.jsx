...
export default function Refund() {
  ...
  const handleSelectSale = async (saleId) => {
    try {
      ...
      logger.log("[Refund] Raw sale data loaded:", full.data);
      if (Array.isArray(full.data?.items)) {
        logger.log("[Refund] Raw items:", full.data.items);
      }
      ...
      if (allZero && (full.data.items || []).length > 0) {
        logger.warn("All item prices are 0; review sale data:", full.data);
        alert(
