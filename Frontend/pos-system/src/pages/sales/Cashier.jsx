...
// Inline API client for this page (avoid shared axiosSetup per request)
      ...
      if (code === "SHIFT_OUTSIDE") {
        try {
          if (url.includes("/api/auth/login")) {
            ...
            window.alert(
              "คุณอยู่นอกเวลางานแล้ว ไม่สามารถใช้งานระบบได้"
            );
...
function CashierInner() {
  ...
  // ===== TYPEAHEAD (debounced suggestions) =====
  useEffect(() => {
  ...
  console.log("[PromptPay] create intent", {
      ...
      pollTimerRef.current = setInterval(async () => {
        try {
          ...
          setQrStatus(status || "");
          console.log("[PromptPay poll]", { status });
              ...
              const salePoll = setInterval(async () => {
                try {
                  ...
                  if (e?.response?.status === 404) {
                    ...
                    console.error("Sale lookup error:", e);
                  }
                }
              }, 2000);
            }
          ...
          console.error("PromptPay poll error:", pollErr);
        }
      }, 2000);
    } catch (err) {
  ...
  console.log("[Card] create intent", { total: Number(total.toFixed(2)) });
  ...
  console.error("Card flow error:", err);
      alert(msg);
      setIsProcessing(false);
    }
  ...
  console.error("Checkout failed:", status, msg, err?.response?.data);
  console.log("[Cashier] last payload:", {
        cart,
        paymentMethod,
        amountReceived,
      });
  ...
  const handlePrintPDF = async () => {
    ...
    try {
      // Optional embedded Thai font; will be skipped if not provided
      // Define global THSarabunNew in your app to enable this embedding.
          ...
          >
            <div style={{ position: "relative", flex: 1 }}>
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    if (suggestOpen && highlightIndex >= 0 && suggestions[highlightIndex]) {
                  ...
                  {suggestions.map((s, idx) => (
                    <div
                      ...
                      style={{
                        padding: "8px 10px",
                        cursor: "pointer",
                        background: idx === highlightIndex ? "#f0fdf4" : "#fff",
                        display: "flex",
                        justifyContent: "space-between",
                        alignItems: "center",
                    ...
                    >
                      ...
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 13 }}>{safeToFixed(s.price)} ฿</div>
                        {isFiniteNumber(s.stock) && (
