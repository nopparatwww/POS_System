...
export default function LogsView({ title = 'Activity Logs', fixedRole = null, endpoint = '/api/protect/logs' }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // Build endpoint: allow overriding base path; if fixedRole provided and endpoint is the generic base, append segment
        let url = ''
        if (endpoint && endpoint !== '/api/protect/logs') {
          url = `${API_BASE}${endpoint}?${params.toString()}`
        } else {
          const path = fixedRole ? `/api/protect/logs/${fixedRole}` : '/api/protect/logs/all'
          url = `${API_BASE}${path}?${params.toString()}`
        }
    ...
    load()
  }, [API_BASE, page, limit, q, user, fixedRole])
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(12px,2vw,24px)' }}>
      <div style={{ width: '100%', maxWidth: 1000 }}>
        <h2 style={{ margin: '4px 0 12px 0' }}>{title}</h2>
