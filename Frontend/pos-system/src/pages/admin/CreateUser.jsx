...
export default function CreateUser() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      try {
        await axios.put(
          `${API_BASE}/api/permissions/${encodeURIComponent(username)}`,
          { allowRoutes, denyRoutes: [], notes: "" }
        );
      } catch (permErr) {
        // If permissions update fails, report but still allow navigation
        logger.error("Failed to set permissions for new user:", permErr);
      }
      alert("User created");
  ...
  return (
    <div style={{ padding: "clamp(12px,2vw,24px)" }}>
      ...
      <h2 style={{ margin: "4px 0 12px 0", textAlign: "left" }}>
        Create new user
      </h2>
      <form
      ...
      >
        ...
        >
          ...
          <div style={{ color: "#64748b", fontSize: 12, marginBottom: 8 }}>
            เมื่อเลือก Role ระบบจะตั้งค่าเริ่มต้นให้อัตโนมัติ
            สามารถปรับติ๊กเพิ่ม/ลดได้
          </div>
          <div
            style={{
              display: "grid",
